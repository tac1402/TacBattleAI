...
_delegate string GetInfoDelegate()_
_GetInfoDelegate GetInfoHandler_
_string GetInfo()_

Указатель на метод, который возвратит информацию для UI об этой точке. Устанавливается объектами уровня Society/World. Для вызова используется вспомогательный метод _GetInfo()_.
