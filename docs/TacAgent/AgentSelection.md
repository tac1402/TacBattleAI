...
__TODO.__ В текущей реализации, в один момент времени, только один агент может быть выбран. Хотя теоретически можно развить этот класс настройкой управления сразу нескольких агентов. 

## Расширение компонента TopCamera

Для реализации своей функциональности компонент __AgentSelection__ __сам__ (код находится в этом же файле, что и сам класс) дополняет компонент TopCamera описанными ниже членами, часть описания приведенно лишь для понимания логики работы компонента и помечено как _internal_, т.к. всю эту функциональность конечному разработчику использовать не нужно. Единственно, что требуется от разработчика - это задать значение свойства слоя агентов у камеры (TopCamera.AgentLayer).
...
_event AgentInfo OnAgentTap_ (_internal_ __+__)
...
### TopCamera.GetAgent()
_GameObject GetAgent(Vector2 touch)_ (_internal_ __+__)
