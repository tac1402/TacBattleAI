...
Для реализации своей функциональности компонент __AgentSelection__ __сам__ (код находится в этом же файле, что и сам класс) дополняет компоненты _TopCamera_ и _Agent_ описанными ниже членами, часть описания приведенно лишь для понимания логики работы компонента и помечено как _protected internal_ (доступ только из той же сборки при использовании класса), т.к. всю эту функциональность конечному разработчику использовать не нужно. Единственно, что требуется от конечного разработчика - это задать значение свойства слоя агентов у камеры (_TopCamera.AgentLayer_).
...
### InitAgent 
_bool InitAgent_ (_protected internal_)
...
_event AgentInfo OnAgentTap_ (_protected internal_)
...
### TopCamera.GetAgent()
_GameObject GetAgent(Vector2 touch)_ (_protected internal_)
