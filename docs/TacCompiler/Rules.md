...
__Важно! Для Unity.__ В Юнити по умолчанию Roslyn-компилятор не может анализировать внутренние члены классов, поэтому чтобы TacCompiler мог выполнить свою задачу необходимо предоставить разрешение для каждой динамической библиотеки (.dll), в которой используются делегаты. Если это сборка по умолчанию (__Assembly-CSharp__), то ниже указанный код может располагаться в любом месте кода, а если это библиотека определенная через .asmdef, то там же где лежит это объявление.
